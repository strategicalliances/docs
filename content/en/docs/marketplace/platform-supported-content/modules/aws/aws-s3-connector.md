...
From version 5.1, the connector also supports [Amazon S3 client-side encryption]([https://marketplace.mendix.com/link/component/120340](https://docs.aws.amazon.com/amazon-s3-encryption-client/latest/developerguide/what-is-s3-encryption-client.html). To use this client-side encryption, an [AWS KMS key](https://docs.aws.amazon.com/kms/latest/developerguide/overview.html) is needed. It is possible to use different credentials for getting the KMS key and storing or retrieving the S3 object. To use this client side encryption, you have set the KMS key, the region and the KMS credentials in the new 'ClientsideEncryptionConfig' entity and associate it with the PutObjectRequest and GetObjectRequest.

From version 5.2 the connector has been updated to support the new Amazon V4 client-side encryption. This new encryption algorithm is not backwards compatible with the previous, V3 encryption algorithm. Thereofor Aaazon has introduced a 'CommitmentPolicy' to support multiple migration scenario's. Our connector supports this as well. However the connecotr defaults to the new, most secure, V4 encryption algorithm. So when you have to migrate, you have to change the 'CommitmentPolicy' attribute in the 'ClientsideEncryptionConfig' entity according to [Amazon's migration documentation](https://docs.aws.amazon.com/amazon-s3-encryption-client/latest/developerguide/java-v4-migration.html).
